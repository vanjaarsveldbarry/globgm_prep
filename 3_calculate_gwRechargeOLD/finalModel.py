...
dataFolder = Path('/projects/0/einf4705/gwRecharge_correction/data_processed')
outputFolder= Path('/projects/0/einf4705/gwRecharge_correction/output')
...
with dask.config.set(**{'array.slicing.split_large_chunks': False}):
    ...
    for file in predicted_recharge_files:
        ...
        if count == 1: 
            ...
            print(ds_final)
        print(f'{count} / {total}')
ds_grid = xr.open_dataset(dataFolder / 'recession_coefficient_30sec.nc')['recession_coefficient_30sec_map']
ds_final = ds_final.sortby('lat', ascending=False)
ds_final = ds_final.reindex(lat=ds_grid.lat, lon=ds_grid.lon, method='nearest')
